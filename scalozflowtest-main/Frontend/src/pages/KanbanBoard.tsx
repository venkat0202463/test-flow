...
const KanbanBoard = () => {
  ...
  const [showActiveSprintOnly] = useState(false);
  ...
  const normalizeStatus = (status?: string, colNames: string[] = []) => {
    if (!status) return colNames[0] || 'To Do';
    ...
    if (reviewStatuses.includes(upperStatus)) {
      return colNames[0] || 'To Do';
    }

    // Match by exact name first, then normalized
    const exactMatch = colNames.find(c => c === status);
    if (exactMatch) return exactMatch;

    const normalized = status.trim().toLowerCase().replace(/[^a-z0-9]/g, '');
    const normalizedMatch = colNames.find(c => c.trim().toLowerCase().replace(/[^a-z0-9]/g, '') === normalized);
    return normalizedMatch || status;
  };
  ...
  const fetchData = async () => {
    ...
    try {
      const [tasksRes, sprintsRes, colRes, projRes, usersRes] = await Promise.all([
  ...
  useEffect(() => {
    if (!loading && project && project.projectType?.toUpperCase() === 'SCRUM') {
      navigate(`/dashboard/sprint-board/${projectId}`);
    }
  ...
  const handleUpdateTaskStatus = async (taskId: number, newStatus: string) => {
    ...
    try {
      const sprintId = (task as any).sprintId ?? (task as any).sprint?.id ?? null;
  ...
  const handleDeleteColumn = async (colId: number) => {
    ...
    const tasksInCol = displayedTasks.filter(t => normalizeStatus(t.status, columns.map(c => c.name)) === column.name);
    ...
    if (tasksInCol.length > 0) {
      ...
      const targetColId = prompt(`This column has ${tasksInCol.length} tasks. Enter the name of the column to move them to: (${otherColumns.map(c => c.name).join(', ')})`);
      if (!targetColId) return;

      const targetCol = otherColumns.find(c => c.name.toLowerCase() === targetColId.toLowerCase());
      if (!targetCol) {
  ...
  const isProjectKanban = (project?.projectType || project?.type || '').toUpperCase().trim() === 'KANBAN';
  ...
  const displayedTasks = tasks.filter(t => {
    ...
    if (isProjectKanban) {
      if (t.environment === 'BACKLOG') return false;
    } else {
      const isInActiveSprint = activeKanban && (t.sprint?.id === activeKanban.id);
      if (showActiveSprintOnly && !isInActiveSprint) return false;
      if (!showActiveSprintOnly && t.environment === 'BACKLOG' && !isInActiveSprint) return false;
    }
  ...
  // Only show "No Active Cycle" for SCRUM projects that have no active sprint.
  // KANBAN projects should always show the board.
  if (!isProjectKanban && !activeKanban && project?.projectType?.toUpperCase() === 'SCRUM') {
    return (
      <div className="flex flex-col items-center justify-center h-full bg-white">
        ...
        <h2 className="text-[24px] font-semibold text-[#172B4D] mb-3 tracking-tighter">No Active Cycle</h2>
        <p className="text-[#5E6C84] text-[14px] font-medium mb-8">Start a sprint or kanban flow from the backlog to begin.</p>
        <button
          onClick={() => window.location.href = `/dashboard/backlog/${projectId}`}
          className="bg-[#1F6FEB] text-white text-[14px] font-bold px-6 py-2.5 rounded hover:bg-[#003484] transition-colors shadow-md shadow-blue-500/20"
        >
          Go to Backlog
        </button>
      </div>
    ...
    <div className="board-container h-full bg-white flex flex-col overflow-hidden animate-in fade-in duration-500">
      <div className="px-8 pt-6 pb-4 border-b border-[#DFE1E6]">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="section-subtitle mt-1 tracking-tight">Global Workflow • Continuous Delivery Node</p>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-2 bg-[#F4F5F7] p-1 rounded-[3px] border border-[#DFE1E6]">
              <button
            ...
            <button
              ...
              className="flex -space-x-2 hover:opacity-80 transition-opacity p-1 rounded-lg hover:bg-gray-50 group"
              title="View all team members"
            >
              {(project?.teamMembers || []).map((member: any) => {
                const memberName = (member?.name || '').trim() || 'Team Member';
                return (
                  <div key={member.id} title={memberName} className="w-8 h-8 rounded-full bg-[#1F6FEB] border-2 border-white flex items-center justify-center text-[10px] font-black text-white uppercase shadow-sm cursor-pointer hover:z-10 transition-transform hover:scale-110">
              ...
              {project?.createdBy && (
                (() => {
                  const leadName = (project.createdBy.name || '').trim() || 'Project Manager';
                  return (
                    <div title={`${leadName} (Lead)`} className="w-8 h-8 rounded-full bg-[#00B3A4] border-2 border-white flex items-center justify-center text-[10px] font-black text-white uppercase shadow-sm cursor-pointer ring-2 ring-[#00B3A4]/10 hover:z-10 transition-transform hover:scale-110">
          ...
          <div className="flex gap-6 h-fit min-h-full min-w-max pb-4">
            {columns.map(col => {
              ...
              return (
                ...
                >
                  <div className="kanban-column-header board-column-header flex items-center justify-between px-3 py-2 bg-[#1F6FEB] border border-[#DFE1E6]/10 rounded-t-lg group cursor-move">
                    {editingColumnId === col.id ? (
                      ...
                      <>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-bold text-white uppercase tracking-widest">{col.name}{colTasks.length > 0 ? ` (${colTasks.length})` : ''}</span>
                          {isManager && (
                ...
                {!isAddingColumn ? (
                  ...
                  <div className="p-3 bg-white border border-[#DFE1E6] rounded-lg shadow-sm">
                    ...
                    <div className="flex items-center gap-3">
                      <button onClick={handleAddColumn} className="bg-[#1F6FEB] text-white px-3 py-1 text-xs rounded hover:bg-[#003484] font-bold">Add</button>
      ...
      {isIssueModalOpen && (
        <CreateIssueModal
          ...
          onSuccess={() => { fetchData(); setIsIssueModalOpen(false); setIssueModalStatus(null); }}
